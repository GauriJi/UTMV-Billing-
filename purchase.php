...
=======
            $purchase_no, $supp_id, $supp_manual, $_POST['purchase_date'],
...
=======
        ...
        .product-suggestions {
            ...
            box-shadow:0 4px 12px rgba(0,0,0,.15); max-height:280px;
            overflow-y:auto; z-index:1000; margin-top:4px; display:none;
        }
        .product-suggestions.active { display:block; }
        .product-suggestion-item { padding:10px 14px; cursor:pointer; border-bottom:1px solid #f1f5f9; transition:background .2s; }
        .product-suggestion-item:hover { background:#f8fafc; }
        .product-suggestion-item:last-child { border-bottom:none; }
        .product-name { font-weight:600; color:#1e293b; margin-bottom:3px; }
        .product-details { font-size:11px; color:#64748b; }
        .product-details span { margin-right:10px; }
        .no-results { padding:14px; text-align:center; color:#94a3b8; font-size:13px; }
        ...
        #supp_drop {
            ...
            box-shadow:0 6px 16px rgba(0,0,0,.12); max-height:220px;
...
>>>>>>> 2ccbacb326bbaaafa6cad901234786a0455599fd
        ...
        /* Compact inputs inside items table */
...
include 'topbar.php'; ?>
        <div class="content-wrapper">
            ...
            <?php if (isset($_SESSION['success'])): ?><div class="alert alert-success"><?php echo $_SESSION['success'];
    unset($_SESSION['success']); ?></div><?php
endif; ?>
            ...
            <form id="purchaseForm" method="POST" class="invoice-form">
                ...
                <div id="gst_type_row" style="display:none;align-items:center;gap:12px;background:#fef9c3;border:1px solid #fbbf24;border-radius:8px;padding:12px 16px;margin-bottom:20px;">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:20px;">
                    <div style="background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:10px;padding:16px;">
                        ...
                        <input type="text" name="bill_address" id="bill_address" class="form-control" placeholder="Street / Area" oninput="syncShip()"></div>
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px;">
                            <div><label style="font-size:12px;">City</label><input type="text" name="bill_city" id="bill_city" class="form-control" placeholder="City" oninput="syncShip()"></div>
                            <div><label style="font-size:12px;">State</label><input type="text" name="bill_state" id="bill_state" class="form-control" placeholder="State" oninput="syncShip()"></div>
                        </div>
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px;">
                            <div><label style="font-size:12px;">Pincode</label><input type="text" name="bill_pincode" id="bill_pincode" class="form-control" placeholder="Pincode" oninput="syncShip()"></div>
                            <div><label style="font-size:12px;">Phone</label><input type="text" name="bill_phone" id="bill_phone" class="form-control" placeholder="Phone" oninput="syncShip()"></div>
                        </div>
                        <div><label style="font-size:12px;">GSTIN</label><input type="text" name="bill_gstin" id="bill_gstin" class="form-control" placeholder="GSTIN/UIN" oninput="syncShip()"></div>
                    </div>
                    <div style="background:#f0fdf4;border:1.5px solid #bbf7d0;border-radius:10px;padding:16px;">
                        ...
                        <div class="form-group" style="margin-bottom:10px;"><label style="font-size:12px;">Consignee Name</label><input type="text" name="ship_name" id="ship_name" class="form-control" placeholder="Name of consignee"></div>
                        <div class="form-group" style="margin-bottom:10px;"><label style="font-size:12px;">Address</label><input type="text" name="ship_address" id="ship_address" class="form-control" placeholder="Street / Area"></div>
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px;">
                            <div><label style="font-size:12px;">City</label><input type="text" name="ship_city" id="ship_city" class="form-control" placeholder="City"></div>
                            <div><label style="font-size:12px;">State</label><input type="text" name="ship_state" id="ship_state" class="form-control" placeholder="State"></div>
                        </div>
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px;">
                            <div><label style="font-size:12px;">Pincode</label><input type="text" name="ship_pincode" id="ship_pincode" class="form-control" placeholder="Pincode"></div>
                            <div><label style="font-size:12px;">Phone</label><input type="text" name="ship_phone" id="ship_phone" class="form-control" placeholder="Phone"></div>
                        </div>
                        <div><label style="font-size:12px;">GSTIN</label><input type="text" name="ship_gstin" id="ship_gstin" class="form-control" placeholder="GSTIN/UIN"></div>
                    </div>
                </div>
...
=======
                ...
                <div style="background:#fefce8;border:1.5px solid #fde68a;border-radius:10px;padding:16px;margin-bottom:20px;">
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;">
                        <div><label style="font-size:12px;">Delivery Note No.</label><input type="text" name="delivery_note" class="form-control" placeholder="Delivery note no."></div>
                        <div><label style="font-size:12px;">Supplier's Order No.</label><input type="text" name="buyer_order_no" class="form-control" placeholder="Order number"></div>
                        <div><label style="font-size:12px;">Dispatch Doc No.</label><input type="text" name="dispatch_doc_no" class="form-control" placeholder="Dispatch doc no."></div>
                        <div><label style="font-size:12px;">Dispatched Through</label><input type="text" name="dispatched_thru" class="form-control" placeholder="e.g. DTDC, By Hand"></div>
                        <div><label style="font-size:12px;">Destination</label><input type="text" name="destination" class="form-control" placeholder="Destination city"></div>
                    </div>
                </div>
                    ...
                    <table class="data-table">
                        <thead>
                            <tr>
                                ...
                                <th>Amount</th><th>GST</th><th>Grand Total</th><th>Status</th><th style="width:130px">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php if (empty($recent_purchases)): ?>
                            <tr><td colspan="8" class="no-data">No purchases found</td></tr>
                            <?php
else:
    foreach ($recent_purchases as $p):
        $status_colors = ['paid' => '#dcfce7;color:#166534', 'partial' => '#fef9c3;color:#713f12', 'pending' => '#fee2e2;color:#991b1b'];
...
?>
                            <tr>
                                <td><strong><?php echo htmlspecialchars($p['purchase_no']); ?></strong></td>
                                <td><?php echo htmlspecialchars($p['supplier_name'] ?? $p['supplier_name_manual'] ?? 'N/A'); ?></td>
                                <td><?php echo date('d M Y', strtotime($p['purchase_date'])); ?></td>
                                <td>₹<?php echo number_format($p['total_amount'], 2); ?></td>
                                <td>₹<?php echo number_format($p['cgst_amount'] + $p['sgst_amount'] + $p['igst_amount'], 2); ?></td>
                                <td><strong>₹<?php echo number_format($p['grand_total'], 2); ?></strong></td>
                                <td><span style="padding:3px 10px;border-radius:12px;font-size:11px;font-weight:600;background:<?php echo $sc; ?>"><?php echo ucfirst($p['payment_status'] ?? 'pending'); ?></span></td>
                                <td style="white-space:nowrap;">
                                    <a href="edit_purchase.php?id=<?php echo $p['id']; ?>" class="btn btn-secondary" style="padding:4px 10px;font-size:12px;margin-right:4px;">✏️ Edit</a>
                                    <a href="purchase_print.php?id=<?php echo $p['id']; ?>" target="_blank" class="btn btn-secondary" style="padding:4px 10px;font-size:12px;">🖨️ Print</a>
                                </td>
                            </tr>
                            <?php
    endforeach;
